...
function ScoreRow({ label, value, isEditing, onChange }) {
...
function ScoringRulesCard({ scoring, isEditingScoring, isSavingScoring, onChange, onEdit, onCancel, onSave }) {
  return (
    <article className="rounded-2xl border bg-card p-4 lg:col-span-5">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <section className="rounded-xl bg-accent border p-3">
          ...
          <ScoreRow label="Correct" value={scoring.scoreCorrect} isEditing={isEditingScoring} onChange={(value) => onChange({ scoreCorrect: value })} />
          <ScoreRow label="Incorrect" value={scoring.scoreIncorrect} isEditing={isEditingScoring} onChange={(value) => onChange({ scoreIncorrect: value })} />
        </section>
        <section className="rounded-xl bg-accent border p-3 md:row-span-2">
          ...
          <ScoreRow label="Win" value={scoring.scoreChallengeWinner} isEditing={isEditingScoring} onChange={(value) => onChange({ scoreChallengeWinner: value })} />
          <ScoreRow label="Loss" value={scoring.scoreChallengeLoser} isEditing={isEditingScoring} onChange={(value) => onChange({ scoreChallengeLoser: value })} />
          <div className="mt-3 rounded-xl border-2 border-dashed -m-1.5 p-2">
            ...
            <ScoreRow label="Attacker" value={scoring.scoreChallengeDrawAttacker} isEditing={isEditingScoring} onChange={(value) => onChange({ scoreChallengeDrawAttacker: value })} />
            <ScoreRow label="Defender" value={scoring.scoreChallengeDrawDefender} isEditing={isEditingScoring} onChange={(value) => onChange({ scoreChallengeDrawDefender: value })} />
          </div>
        </section>
        <section className="rounded-xl bg-accent border p-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Penalty tile</p>
          <ScoreRow label="Penalty" value={scoring.scoreAttack} isEditing={isEditingScoring} onChange={(value) => onChange({ scoreAttack: value })} />
        </section>
      </div>
    </article>
  )
