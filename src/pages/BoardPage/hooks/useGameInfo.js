...
import { fetchInitialGameData, updateGame, updateTeam, insertAnswer } from '@/services/api'
...
export function useGameInfo() {
  ...
  useEffect(() => {
    const loadGameData = async () => {
      try {
        const data = await fetchInitialGameData();
        setBoard(data.board)
        setGame(data.game)
        setTeams(data.teams)
  ...
  const updateTurn = async (teamId) => {
    ...
    try {
      await updateGame(game.id, { current_team_id: teamId })
    } catch (error) {
  ...
  const updateScore = async (teamId, pointsDelta) => {
    ...
    // Sincronizar DB
    try {
      await updateTeam(teamId, { score: newScore });
    } catch (error) {
  ...
  const updatePosition = async (teamId, newPosition) => {
    ...
    try {
      await updateTeam(teamId, { position: newPosition })
    } catch (error) {
  ...
  return {
    board, teams, answers, categories, boardCategory, questions, //game
    isLoading, loadError,
